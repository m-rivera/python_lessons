...
What now follows is a long program (by the standards of these lessons) which
reads all the lines of this file which start with a number (which allows us to
skip the header). The data is stored by the program and the last x and y data
points are printed out as a health check.
