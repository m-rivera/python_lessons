...
````
<details> <summary>Solution</summary>
In **a**, the order of operations has been treated poorly, so the answer is incorrect.<br>
In **b**, the numbers have been turned into strings, and then divided by `3`. Strings can't be divided by integers, which raises `TypeError`.<br>
In **c**, the strings have been turned into numbers, except Python isn't sure how to turn `"12.0"` into an integer due to the decimal place, even though it is followed by `0`. This raises a new, very common error: `ValueError`.<br>
