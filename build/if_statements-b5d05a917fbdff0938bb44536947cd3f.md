...
```
First, note the the syntax of the line `if molecule_a == molecule_b:`. The word `if` is reserved in Python, so should appear in a different colour in your editor. There is then a space and what follows is anything that resolves to a Boolean; here, a comparison.

Then, some code appears with 4 leading spaces:
```Python
    # This part of the code is run only if the if statement is true
    print("Yes, these molecules have the same formula.")
    print(f"This formula is {molecule_a}.")
