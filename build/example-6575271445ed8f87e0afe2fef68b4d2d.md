...
Replace this line:
```python
    if (absorbances[i+1] < absorbances[i+2]) and (absorbances[i+3] <
absorbances[i+2]):
```
with the following:
    if (absorbances[i] < absorbances[i+2]) and (absorbances[i+4] <
absorbances[i+1]):
```
...
plt.savefig("specturm.pdf")

```

Now search your working folder for a file called `specturm.pdf` and open it. Try
